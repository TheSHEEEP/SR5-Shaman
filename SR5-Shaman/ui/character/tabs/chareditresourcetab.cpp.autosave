...
CharEditResourceTab::initialize()
{
    ...
    // Available resources
    _resourceModelAvailable = new ResourceTreeModel(true);
    _resourceModelAvailable->setItemView(ui->treeResourcesAvailable);
    _resourceModelAvailable->initialize();
    // Filter
    _resourceFilterAvailable = new ResourceSortFilterProxyModel(ui->treeResourcesAvailable);
    _resourceFilterAvailable->setSourceModel(_resourceModelAvailable);
    /*std::vector<SkillType> filterTypes;
    filterTypes.push_back(SKILL_TYPE_COMBAT);
    filterTypes.push_back(SKILL_TYPE_KNOWLEDGE);
    filterTypes.push_back(SKILL_TYPE_LANGUAGE);
    ...
    _skillsFilter->setShowEmptyCategories(true);
    _skillsFilter->setShowUserSkills(true);*/
    _resourceFilterAvailable->applyFilter();
    // Model & sorting
    ui->treeResourcesAvailable->setModel(_resourceFilterAvailable);
    ui->treeResourcesAvailable->setSortingEnabled(true);
    ui->treeResourcesAvailable->setEditTriggers(QAbstractItemView::AllEditTriggers);
    // Delegate
    _resourceDelegate = new ResourceDelegate();
    ui->treeResourcesAvailable->setItemDelegate(_resourceDelegateAvailable);
    /*connect(_resourceDelegate, SIGNAL(skillChanged()), SLOT(handleSkillValueChanged()));
    connect(_resourceDelegate, SIGNAL(addButtonClicked(SkillDefinition*)),
                               SLOT(handleCustomSkillAdd(SkillDefinition*)));
    connect(_resourceDelegate, SIGNAL(specializationsClicked(SkillDefinition*)),
                               SLOT(manageSpecializations(SkillDefinition*)));*/
    // Handle selection & drag
    connect(ui->treeResourcesAvailable->selectionModel(),  SIGNAL(currentChanged(QModelIndex,QModelIndex)),
                                                           SLOT(handleResourceChanged(QModelIndex,QModelIndex)));

    // Insert resource priorities
